function [bestC,bestG,bestE,Beta,ker]=pso_search(trainX,trainY,range,c1,c2,wRange,k,tol)
...
%                  2^-3,2^2;
%                  10^-4,10^-1];
%           %%待寻优参数的范围,是一个particleSize行，2列的矩阵，其中，第一列为min，第二列为max。每行代表一个参数�?依次为C，Gamma，epsilon
%   c1,c2
...
fittnessAtGen=zeros(maxGen+1,1);%全局�?��适应度的收敛状况
results=zeros(particleSize*maxGen,4);
%% 初始化粒子群
parfor i=1:particleSize
    for j=1: dimension %待寻参数
       
        position(i,j)=rand()*(range(j,2)-range(j,1))+range(j,1);%rand*（最�?�?���?�?��
...
while i<=maxGen %% TC<=20
    ...
    if gBest<gBestFittness %如果有更优的全局点，更新全局�?���?
        ...
        % Ktrain = kernelmatrix('rbf',testX',trainX',bestG);
        % Ypred = Ktrain*Beta;
        % [accuracy] = mape(testY,Ypred);
